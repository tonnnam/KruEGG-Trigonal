...
def parse_answer(user_input: str):
    """แปลงคำตอบผู้เล่นเป็น float รองรับ √ และเศษส่วน"""
    user_input = user_input.strip().replace("√", "math.sqrt")
...
def check_answer(user_input, correct_value, tol=0.02):
    """ตรวจคำตอบ ใช้ tolerance"""
...
# ใช้ key ตาม level เพื่อให้ input ล้างทุกครั้ง
form_key = f"answer_form_{st.session_state.level}"
input_key = f"answer_input_{st.session_state.level}"
